...
# Handling the Control FIlet
#
#In addition handle stale PIds, rotate logs ect
#
base_dir="$HOME/.local/run/lcsession"
mkdir -p "$base_dir"
log_dir="$HOME/.local/log/lcsession"
mkdir -p "$log_dir"
log_archive_dir="$log_dir/logs_archive"
...
rotate_log() {
  log_file="$1"
  archive_dir="$2"
  if [[ -f "$log_file" && $(stat -c %s "$log_file") -gt 100000 ]]; then
    timestamp=$(date +%F_%T)
...
start_clipboard() {
  (
    while true; do
      rotate_log "$clip_log" "$log_archive_dir"
      {
...
start_dirwatch() {
  ...
  (
    while true; do
      rotate_log "$dir_log" "$log_archive_dir"
...
cleanup_pid() {
  if [[ -f "$clip_pid" && ! -d /proc/$(cat "$clip_pid") ]]; then
    rm -f "$clip_pid"
  fi
  if [[ -f "$dir_pid" && ! -d /proc/$(cat "$dir_pid") ]]; then
    rm -f "$dir_pid"
  fi
}
